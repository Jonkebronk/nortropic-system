...
2. Execute your full build process from the brief:
   ...
   - Copy `nortropic-bild/scripts/treatment.mjs` into the project as `scripts/treatment.mjs`, add `sharp` as a devDependency, and wire `"prebuild": "node scripts/treatment.mjs"` in package.json (kör `--stage=both`: `raw/` → `ref/` → `public/images/`). Lägg `public/images/ref/` i `.vercelignore` — internt mellanmaterial, deployas aldrig (samma behandling som `design-referenser/`). Skriv de två kanoniska palett-aliasen i `src/app/globals.css` `:root`: `--nortropic-ink` (mörkaste ytan i paletten) och `--nortropic-accent` (trade-anchored accent ur §5) — alias till befintliga tokenvärden, aldrig nya färger. treatment.mjs läser palett ur globals.css och preset ur SLOTS.json — never hardcode colors or preset.
