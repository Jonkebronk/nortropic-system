...
| (b) Tidsfälla | Submits < 3 s efter sidladdning avvisas (tyst). Saknat/0-värde på `startedAt` (direkt-POST förbi formuläret) är INTE ett fynd så länge honeypot + validering finns — fältet är ett bot-filter, inte en autentisering | `startedAt` sätts client-side vid mount, valideras server-side: `Date.now() - startedAt < 3000` → tyst ok |
