...
## Process
1. Scope: the changed pages/components if given, else the whole site. Läs `PROJECT-BRIEF.md` §5 Design Direction (**Vald riktning** och **Motion-nivå**) och **§7 Kalibreringsprofil** (primärhandling, röstregister, bransch-antislop, kvittolista) — eller `content/profile.ts` när endast byggrepot finns. Saknar en äldre brief Motion-nivå-fältet: behandla som `subtil` och notera det. Saknar den §7: granska mot hantverkar-defaulten och notera det.
