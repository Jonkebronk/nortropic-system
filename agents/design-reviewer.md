...
2. **Ladda designkanonen (obligatoriskt — invoke SAMTLIGA via Skill tool):** `web-design-guidelines`, `ui-ux-pro-max`, `taste`, `impeccable`, `soft-skill`, `emil-design-eng`, `find-animation-opportunities`, `frontend-design`. Granskaren behåller HELA kanonen inklusive de dömande skillsen (`taste`, `impeccable`) — byggarna laddar endast de generativa; oberoendet kräver att du dömer med böcker byggaren inte skrivit själv.
